<div>
    ...
    @endif
    <div>
        <h1 class="text-2xl font-bold">Hutang </h1>
        <p class="text-sm text-gray-500 dark:text-neutral-400">
            Daftar hutang teman kamu yang tercatat.
        </p>
    </div>
                        ...
                        <tbody class="bg-white dark:bg-zinc-900 divide-y divide-gray-200 dark:divide-neutral-700">
                            ...
                            @forelse ($hutang as $index => $item)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200 cursor-pointer"
                                        wire:click="edit('{{ $item->id }}')">
                                        {{-- Sesuai gambar kolom: jumlah --}}
                                        Rp {{ number_format($item->jumlah, 0, ',', '.') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200 cursor-pointer"
                                        wire:click="edit('{{ $item->id }}')">
                                        <span
                                            class="inline-flex items-center gap-x-1.5 py-0.5 px-3 rounded-full text-xs font-medium bg-teal-100 text-teal-800 dark:bg-teal-800/30 dark:text-teal-500">
                                            {{-- Sesuai gambar kolom: metode_pembayaran --}}
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200 cursor-pointer"
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center gap-x-1.5 py-0.5 px-3 rounded-full text-xs font-medium {{ $statusColor }}">
                                            {{ $item->status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200 cursor-pointer"
                                        wire:click="edit('{{ $item->id }}')">
                                        {{-- Sesuai gambar kolom: catatan --}}
                                        {{ Str::limit($item->catatan, 30) }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200 cursor-pointer"
                                        wire:click="edit('{{ $item->id }}')">
                                        {{-- Sesuai gambar kolom: tanggal_pinjaman --}}
                                        {{ \Carbon\Carbon::parse($item->tanggal_pinjaman)->translatedFormat('l, d M Y') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                        <button wire:click="edit('{{ $item->id }}')"
                                            class="text-blue-600 hover:text-blue-800 dark:text-blue-500 mr-2">
